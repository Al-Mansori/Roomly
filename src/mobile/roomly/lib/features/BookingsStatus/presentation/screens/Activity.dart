...
class _ActivityScreenState extends State<ActivityScreen> {
  ...
  BookingCardType _getCardType() {
    switch (selectedTab) {
      case 0: return BookingCardType.upcoming;
      case 1: return BookingCardType.ongoing;
      case 2: return BookingCardType.pending;
      case 3: return BookingCardType.history;
      default: return BookingCardType.ongoing;
    }
  }
                ...
                child: BlocBuilder<BookingsCubit, BookingsState>(
                  builder: (context, state) {
                    if (state is BookingsLoading) {
                      ...
                      final List<BookingWithRoom> filteredBookings = _filterBookings(state.bookings);
  ...
  List<BookingWithRoom> _filterBookings(List<BookingWithRoom> bookings) {
    switch (selectedTab) {
      case 0: // Upcoming
        return bookings.where((b) => b.reservation.reservationDate.isAfter(DateTime.now()) && b.reservation.status != 'CANCELLED').toList();
      case 1: // On-going
        return bookings.where((b) => b.reservation.startTime.isBefore(DateTime.now()) && b.reservation.endTime.isAfter(DateTime.now()) && b.reservation.status != 'CANCELLED').toList();
      case 2: // Pending
        return bookings.where((b) => b.reservation.status == 'PENDING').toList();
      case 3: // History
        return bookings.where((b) => b.reservation.endTime.isBefore(DateTime.now()) || b.reservation.status == 'CANCELLED').toList();
      default: return [];
    }
  }
  ...
  Widget _buildBookingsList(List<BookingWithRoom> bookings) {
    if (bookings.isEmpty) {
      return Center(child: Text('No ${tabs[selectedTab].toLowerCase()} bookings found', style: const TextStyle(fontSize: 16)));
    }
    return ListView.builder(
      ...
      itemBuilder: (context, index) => BookingCard(
        ...
        type: _getCardType(),
        onCancel: () {}, // TODO: Implement
        onModify: () {}, // TODO: Implement
        onRebook: () {}, // TODO: Implement
      ),
    );
  }
