...
import '../../../GlobalWidgets/app_session.dart';
import '../models/loyalty_points_model.dart';
import '../models/room_loyalty_model.dart';
...
abstract class LoyaltyPointsRemoteDataSource {
  ...
  Future<bool> addPoints(String userId, int points);
  Future<bool> redeemPoints(String userId, int points);
  Future<List<TopRoomModel>> getTopRooms();
}
...
class LoyaltyPointsRemoteDataSourceImpl implements LoyaltyPointsRemoteDataSource {
  ...
  @override
  Future<List<TopRoomModel>> getTopRooms() async {
    try {
      final token = AppSession().token;
      if (token == null) {
        throw Exception('No auth token found');
      ...
      final url = Uri.parse('$baseUrl/api/customer/rooms/top');
      ...
      if (response.statusCode == 200) {
        ...
        return jsonData.map((e) => TopRoomModel.fromJson(e as Map<String, dynamic>)).toList();
      } else if (response.statusCode == 404) {
        throw NoDataException(message: 'No top rooms found');
      } else {
        throw Exception('Failed to load top rooms: ${response.statusCode}');
      }
    } catch (e) {
      if (e is NoDataException) {
        rethrow;
      } else {
        throw Exception('Error fetching top rooms: $e');
      }
    }
  }
}
